...
// gunjae: block I/O is processed here
int nvme_bio_request (nvme_bio *bio)
{
	...
	// gunjae: FPGA_DDR_NS=2, FPGA_NAND_NS=4, LS2_DDR_NS=8
	GK_REQ_PRINT("REQ(BIO): req[%s] sLBA[0x%lX] nLB[%d] prp[0x%lX] size[%d] ns[%d] mem_type[%x]\n",
			bdbm_reqtype_name[bio->req_type], bio->slba[0], bio->nlb, bio->prp[0], bio->size, bio->ns_num, g_NvmeCtrl.target.mem_type[bio->ns_num]);

	if (g_NvmeCtrl.target.mem_type[bio->ns_num] & LS2_DDR_NS) {	// gunjae: ramdisk mode
	...
	} else if (g_NvmeCtrl.target.mem_type[bio->ns_num] & FPGA_DDR_NS) {	// gunjae: DDR in FPGA
		//GK_PRINT("REQ(BIO): FPGA_DDR_NS\n");
#if 1
			...
			for(; i < (nlba_def - 1); i++, count++, prp_def++, --nlp) {
				if(((*(prp_def + 1)) - (*prp_def)) != 0x1000) {
					...
					break;
                        ...
                        size = count << PAGE_SHIFT;
						// gunjae: DMA commands are generated here
	...
	} else if (g_NvmeCtrl.target.mem_type[bio->ns_num] & FPGA_NAND_NS) {	// gunjae: NAND in FPGA
		// gunjae: if nand flash is accessed, dma_mgr communicates with FTL by calling host_block_make_req()
...
{
	GK_PRINT("GK: block IO initialization, nvme_bio_init()::");
	int ret = 0;
