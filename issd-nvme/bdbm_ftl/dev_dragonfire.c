...
static void bdbm_find_last_good_block_in_chip (bdbm_nandaddr_t *spare, local_mem_t *lmem)
{
	...
	sem_init (&waiter, 0, 0);
	//GK_PRINT("GK: bdbm_find_last_good_block_in_chip()...\n");
	while (spare->row.block_no != (uint64_t)(-1)) {
		//GK_PRINT("GK: row[%lu] ", spare->row.block_no);
	...
	if (spare->row.block_no == (uint64_t)(-1)) {
		GK_PRINT("GK: no good block found, spare->row.block_no = %lu", spare->row.block_no);
...
int setup_new_ssd_context (bdbm_dev_private_t *pdp)
{
	...
	GK_PRINT("GK: setup_new_ssd_context(), dimm_status=%d\n", pdp->dimm_status);
...
{
	GK_PRINT("GK: bdbm_get_dimm_status()::\n");
	...
	// gunjae
	if (ret < 0) GK_PRINT("GK: DIMM is not detected\n");
	bdbm_bug_on (ret < 0);
	...
	GK_PRINT("GK: row.block_no[%d, %d], col[%d]\n", spare.row.block_no, pdp->nand.nr_blocks_per_chip - 1, pdp->nand.page_main_size);

	bdbm_alloc_pv (&lmem, pdp->nand.page_oob_size);

NEXT_DIMM:
	// gunjae: where is read?
	GK_PRINT("GK: paddr for magic_key (before find), "); print_bdbm_paddr(spare.row); printf("\n");
	bdbm_find_last_good_block_in_chip (&spare, &lmem);
	GK_PRINT("GK: paddr for magic_key (after find), "); print_bdbm_paddr(spare.row); printf("\n");
	if (!magic_done) {
		/* first (and only?) dimm */
		spare_bk = spare;
		magic_done = (*(uint32_t *)lmem.virt == MAGIC_KEY)?1:0;
		GK_PRINT("GK: 1st dimm magic_key = 0x%X (done: %d)\n", *(uint32_t *)lmem.virt, magic_done);
	} else {
		/* second dimm present */
		GK_PRINT("GK: 2nd dimm magic_key = 0x%X (done: %d)\n", *(uint32_t *)lmem.virt, magic_done);
	...
	if (magic_done == 1) {
		...
		if (pdp->dimms.nr_dimms == 2) {
			...
			////////////////////////////////////
			// gunjae: I added this part since I believe block scanning should be started from the end
			spare.row.block_no = pdp->nand.nr_blocks_per_chip - 1;
			spare.col = pdp->nand.page_main_size;
			///////////////////////////////////
			magic_done = 2;
...
int bdbm_badge_dimms (bdbm_dev_private_t *pdp)
{
	...
	int gk_ret = 0;
	uint32_t *tagPtr;
	bdbm_nandaddr_t magic;	// physical addr + offset (col)
	local_mem_t lmem = {0};
	sem_t waiter;

	GK_PRINT("GK: bdbm_badge_dimms():: dimm_status=%d, page_oob_size=0x%X\n", pdp->dimm_status, pdp->nand.page_oob_size);
	...
	GK_PRINT("GK: magic_address: chnl[%d] chip[%d] block[0x%X] page[0x%X] col[0x%X]\n", \
		magic.row.channel_no, magic.row.chip_no, magic.row.block_no, magic.row.page_no, magic.col);
	...
	*(uint32_t *)lmem.virt = MAGIC_KEY;
	...
	// gunjae: 1st dimm erase, added gk_ret as a return value
	gk_ret = df_setup_nand_erase_desc ((void*)&waiter, &magic.row, magic.col, \
			NULL, 0, DF_BIT_MGMT_IO);
	gk_print_nand_desc(magic.row, magic.col, 0, 0, "erase"); GK_PRINT("ret = %d\n", gk_ret);
	//setup_nand_erase_desc (&magic, NULL, 0, &waiter, &ret);
	sem_wait (&waiter);
	bdbm_bug_on (ret != 0);
	/*Send dummy main page write command in order to write on spare area alone*/
	gk_ret = df_setup_nand_wr_desc (NULL, &magic.row, 0, \
			NULL, 0, DF_BITS_MGMT_PAGE);
	gk_print_nand_desc(magic.row, 0, 0, 0, "write"); GK_PRINT("ret = %d\n", gk_ret);
	gk_ret = df_setup_nand_wr_desc ((void*)&waiter, &magic.row, magic.col, \
			lmem.phy, pdp->nand.page_oob_size, DF_BITS_MGMT_OOB);
	gk_print_nand_desc(magic.row, magic.col, 0, pdp->nand.page_oob_size, "write"); GK_PRINT("ret = %d\n", gk_ret);
	...
	if (pdp->dimms.nr_dimms == 2) {
		...
		GK_PRINT("GK: 2nd dimm, magic_blk_idx=%d, dimm_status=%d\n", magicblk2_idx, pdp->dimm_status);
		if (pdp->dimm_status == BDBM_ST_EXIT_GOOD) {
