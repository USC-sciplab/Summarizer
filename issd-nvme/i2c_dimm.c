...
{
    dimm_details *dm;
    // gunjae: 4 DIMM slots are assigned
    ...
    for(device_count = 0; device_count < no_of_dimm; device_count++) {
        ...
        // gunjae: DIMM information is read via i2c bus protocol
        if(!is_i2cdevice_present(i2c1_file,dimm_slot[device_count])) {
            ...
            if(strcmp(spd_buf,"error") == 0) {
                ...
                if((check & 255) != 255) {  /*TODO Check with DDR/NAND device id*/
                    GK_PRINT("GK: DIMM[%d] slot is DDR\n", device_count+1);
                    ...
                    dm->valid = 1;
                    GK_PRINT("GK: DIMM[%d] slot is NAND\n", device_count+1);
