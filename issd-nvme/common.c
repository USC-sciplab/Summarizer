...
#define HOST_OUTBOUND_SIZE  8ULL*1024*1024*1024 /*Outbound iATU size 4GB*/ // gunjae: 4GB? or 8GB???
...
#if (GK_TEST_RD_NAND==2)	// 2nd test by gunjae
//extern unsigned long long int gk_phy_addr[70];
...
#elif (CUR_SETUP == X86_TEST_SETUP)
#define PEX2_HARD_BAR2_OFF   0xF4000000
#define PEX2_HARD_BAR4_OFF   0xFBFF8000
#endif
#endif
...
int mmap_host_mem (HostCtrl *host)
{
	...
	GK_PRINT("GK: host memory region is mapped from 0x%lX with size of 0x%llX\n", (uint64_t)HOST_OUTBOUND_ADDR, HOST_OUTBOUND_SIZE);
...
//uint8_t read_gk_mem_addr()	// can share with gc_phy_addr???
//{
//	struct ad_trans buf;
//	
//	int8_t fd, ret;;
...
//		return -1;
//	}
//	memcpy(gk_phy_addr, buf.phy_addr, sizeof(unsigned long long int)*70);
//	close(fd);
//	return 0;
//}
...
uint8_t* mmap_oob_addr(uint32_t oob_mem_size,int idx)
{
	...
	// gunjae
	memset(virt_addr, 0x00, (oob_mem_size)*getpagesize());
	GK_PRINT("GK: oob_addr[%u]: 0x%llX is mapped with size %u for phy_addr 0x%llX\n", idx, virt_addr, (oob_mem_size)*getpagesize(), offset);
