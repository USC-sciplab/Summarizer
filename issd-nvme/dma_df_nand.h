...
#if ((GK_TEST_RD_NAND==3) || (GK_TEST_RD_NAND==4))	// separate rd
// TOTAL_NAND_DESC_RD should be equal or greater than TOTAL_NAND_DESCRIPTORS
#define NAND_DESC_RD_PER_TABLE	32
#define NAND_DESC_RD_SIZE		8
#define TOTAL_NAND_DESC_RD		(NAND_DESC_RD_PER_TABLE*NAND_DESC_RD_SIZE)
...
enum DESC_TRACK_STAT {
	...
	TRACK_IDX_FETCH = 2,	// data is fetched from FPGA
#endif
};
...
// gunjae: print function
void print_bdbm_paddr(bdbm_phyaddr_t pa);
void gk_print_nand_desc(bdbm_phyaddr_t row, uint32_t col, uint8_t host_addr, uint32_t len, char* cmd);

#define df_print_nand_desc(phy_addr, page_off, host_addr, len, flag) \
		//printf("GK_NAND_DESC: paddr[0x%X], page_off[0x%X], host_addr[0x%X], len[%d], flag[%d]\n", phy_addr, page_off, host_addr, len, flag)
		//((void)0)
//#define GK_PRINT_NAND_DESC(phy_addr, page_off, host_addr, len, cmd) \
//		{ printf("GK_NAND_DESC: "); print_bdbm_paddr(phy_addr); printf("page_off[0x%X], host_addr[0x%X], len[%d], cmd[%s]\n",page_off, host_addr, len, cmd); }
...
typedef struct Desc_Track {
	...
	uint8_t	    is_used;
#if (GK_MEASURE_TIME)
	struct timespec iss_time;	// time stamp
	//clock_t		iss_clks;	// clock stamp
#endif
} Desc_Track;

#if ((GK_TEST_RD_NAND==3) || (GK_TEST_RD_NAND==4))
typedef struct Desc_Rd_Track {
	void		*req_ptr;
	uint8_t		status;
	uint8_t		is_used;
} Desc_Rd_Track;
...
#define DESC_PROC_QUEUE_DEPTH 8

typedef struct Desc_Proc_Track {
	uint8_t		desc_idx;	// descriptor index
	uint32_t	req_type;	// request type
	uint8_t		is_ready;	// ready to processing?
