...
#if (GK_MEASURE_TIME)
	g_DmaCtrl->iss_clks[g_DmaCtrl->head_idx] = clock();
	g_DmaCtrl->desc_opcode[g_DmaCtrl->head_idx] = desc.opcode;
#endif
...
#if (GK_MEASURE_TIME)
	dma->iss_clks = calloc(DESC_TBL_COUNT*DESC_TBL_ENTRIES, sizeof(clock_t));
	dma->desc_opcode = calloc(DESC_TBL_COUNT*DESC_TBL_ENTRIES, sizeof(uint8_t));
#endif
...
#if (GK_MEASURE_TIME)
	FREE_VALID (g_DmaCtrl->iss_clks);
	FREE_VALID (g_DmaCtrl->desc_opcode);
...
WAIT:
				...
				if(csf_bak->dma_cmp){
					...
					#if (GK_MEASURE_TIME)
						ftch_clks = clock();
						iss_clks = g_DmaCtrl->iss_clks[g_DmaCtrl->tail_idx];
						exec_clks = ftch_clks - iss_clks;

						if (g_DmaCtrl->desc_opcode[g_DmaCtrl->tail_idx]==1) {	// read
							n->dram_acc_cnt += 1;
							n->dram_acc_clks += exec_clks;
						}

						if ((n->dram_acc_cnt > 0) && (n->dram_acc_cnt % 1000==0)) {
							clock_gettime(CLOCK_MONOTONIC, &time_end);
							sec_per_acc = (double)(n->dram_acc_clks) / n->dram_acc_cnt / CLOCKS_PER_SEC;
							GK_PRINT("GK_DRAM_CLKS: %ld / %lld, %lf, rate = %lf B/s\n", n->dram_acc_clks, n->dram_acc_cnt, sec_per_acc, (double)KERNEL_PAGE_SIZE/sec_per_acc);
							//bw
							exec_clks = ftch_clks - n->dram_bw_init_clks;
							GK_PRINT("GK_DRAM_BW: %ld / 1000, bw = %lf B/s\n", exec_clks, (double)KERNEL_PAGE_SIZE*1000*CLOCKS_PER_SEC/exec_clks);
							n->dram_bw_init_clks = clock();
